...
nw_matrix_t* nw_create_matrix(
	...
	/* store the gap penalty for this alignment */
	matrix->gapPenalty = gapPenalty;
...
void nw_initialize_matrix(nw_matrix_t* matrix) {
	...
	for (i = 1; i < matrix->columns; i++) {
		matrix->cell[i][0].score  = -i * matrix->gapPenalty;
		matrix->cell[i][0].di	  = -1;
		matrix->cell[i][0].dj	  =  0;
	}

	/* set the score and vector elements of cells in the left column of array	*/
	for (j = 1; j < matrix->rows; j++) {
		matrix->cell[0][j].score  =  -j * matrix->gapPenalty;
		matrix->cell[0][j].di	  =  0;
		matrix->cell[0][j].dj	  = -1;
	}

	matrix->cell[0][0].score = 0;
	matrix->cell[0][0].di = 0;
	matrix->cell[0][0].dj = 0;
}

void nw_score_cell(nw_matrix_t* matrix, int i, int j) {

	/* compute the three possible scores of the current cell */
	int scoreIfGapInA = matrix->cell[ i ][j-1].score  -  matrix->gapPenalty;
	int scoreIfGapInB = matrix->cell[i-1][ j ].score  -  matrix->gapPenalty;
	int scoreIfMatch  = matrix->cell[i-1][j-1].score  +  sm_get_substitution_score(
...
void nw_print_scores(nw_matrix_t* matrix) {
	...
	for (j = 0; j < matrix->rows; j++) {
		...
		for (i = 0; i < matrix->columns; i++) {
			printf("%4d", matrix->cell[i][j].score);
		}
...
void nw_print_vectors(nw_matrix_t* matrix) {
	...
	for (j = 0; j < matrix->rows; j++) {
		...
		// display all elements in the current array row 
		for (i = 0; i < matrix->columns; i++) {
			sprintf(buf, "(%d,%d)", matrix->cell[i][j].di, matrix->cell[i][j].dj);
			printf("%8s", buf);
		}
...
nw_matrix_t* nw_align_sequences(
	...
	nw_matrix_t* matrix = nw_create_matrix(sequenceA, sequenceB, substitution_matrix, gapPenalty);
